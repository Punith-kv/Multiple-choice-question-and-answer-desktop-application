...
public class MCQApplication {
		...
		try {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (options[i].trim().isEmpty()) {
					System.out.println("Error: Option " + (char) ('A' + i) + " cannot be empty.");
					return;
				}
			}
			...
			// Check if the correct option is valid and not empty
			if (correctOptionInput.trim().isEmpty()) {
				System.out.println("Error: Correct option cannot be empty.");
				return;
			}
			...
			do {
				...
				if (!difficultyLevel.equalsIgnoreCase("Easy") && !difficultyLevel.equalsIgnoreCase("Medium")
						&& !difficultyLevel.equalsIgnoreCase("Hard")) {
					System.out.println("Error: Invalid difficulty level. Please enter Easy, Medium, or Hard.");
					difficultyLevel = "";
				}
			} while (difficultyLevel.isEmpty());
			...
			logAction1("ADD", "Added question: " + questionText);
	...
	private static void updateQuestion(Scanner scanner) {
		...
		try {
			...
			Optional<Question> questionOpt = questions.stream().filter(q -> q.getId().equals(questionId)).findFirst();
			...
			if (questionOpt.isPresent()) {
				...
				// Update options
				for (int i = 0; i < 4; i++) {
					System.out.println("Current option " + (char) ('A' + i) + ": " + question.getOptions()[i]);
	...
	public static void deleteQuestion(Scanner scanner) {
		...
		try {
			...
			if (confirm.equalsIgnoreCase("y")) {
				undoStack.push(new ArrayList<>(questions));
				Optional<Question> questionOpt = questions.stream().filter(q -> q.getId().equals(questionId))
						.findFirst();

				if (questionOpt.isPresent()) {
					questions.remove(questionOpt.get());
					logAction1("DELETE", "Deleted question: " + questionOpt.get().getQuestion());
	...
	public static void viewQuestionsByCategory(Scanner scanner) {
		...
		List<Question> filteredQuestions = questions.stream().filter(q -> q.getCategory().equalsIgnoreCase(category))
	...
	public static void viewQuestionsByDifficulty(Scanner scanner) {
		...
		List<Question> filteredQuestions = questions.stream()
				.filter(q -> q.getDifficultyLevel().equalsIgnoreCase(difficulty)).collect(Collectors.toList());
	...
	public static void viewQuestionsByCategoryAndDifficulty(Scanner scanner) {
		...
		Set<String> difficultyLevels = questions.stream().map(Question::getDifficultyLevel).collect(Collectors.toSet());
		...
		List<Question> filteredQuestions = questions.stream().filter(
				q -> q.getCategory().equalsIgnoreCase(category) && q.getDifficultyLevel().equalsIgnoreCase(difficulty))
				.collect(Collectors.toList());
	...
	public static void viewRecentQuestions() {
		...
		questions.stream().sorted((q1, q2) -> q2.getCreatedAt().compareTo(q1.getCreatedAt())).limit(10) // Only print 10
																										// questions
	...
	public static void takeQuiz(Scanner scanner, boolean retake) {
		...
		if (!retake) {
			...
			quizQuestions = questions.stream()
					...
					.filter(q -> selectedDifficulty.isEmpty()
							|| q.getDifficultyLevel().equalsIgnoreCase(selectedDifficulty))
					.collect(Collectors.toList());
		} else {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (parts.length >= 6) {
					...
					questions.add(new Question(questionText, options, correctOption, category, difficulty));
					imported++;
				}
			}
	...
	public static void exportQuestions(Scanner scanner) {
		...
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath))) {
			for (Question q : questions) {
				writer.write(q.getQuestion() + "|" + String.join("|", q.getOptions()) + "|" + q.getCorrectOption() + "|"
						+ q.getCategory() + "|" + q.getDifficultyLevel());
	...
	public static void viewAuditLog() {
		...
		System.out.println("\nAudit Log:");
		auditLogs.forEach(log -> System.out.println(log.timestamp + " - " + log.action + ": " + log.details));
	}
	...
	public static void viewQuizHistory() {
		...
		for (int i = 0; i < quizHistory.size(); i++) {
			...
			System.out.println("\nAttempt " + (i + 1) + ":");
			System.out.println("Category: " + (attempt.category.isEmpty() ? "All" : attempt.category));
			System.out.println("Difficulty: " + (attempt.difficultyLevel.isEmpty() ? "All" : attempt.difficultyLevel));
	...
	public static void displayQuizSummary(QuizAttempt attempt) {
		System.out.println("\nQuiz Summary:");
		System.out.println("Score: " + attempt.getScore() + "/" + attempt.getAnswers().size());
		System.out.println("Percentage: " + (attempt.getScore() * 100.0 / attempt.getAnswers().size()) + "%");

		System.out.println("\nDetailed Answer Review:");
		for (int i = 0; i < attempt.getAnswers().size(); i++) {
			QuizAnswer answer = attempt.getAnswers().get(i);
			Question question = questions.stream().filter(q -> q.getId().equals(answer.questionId)).findFirst()
					.orElse(null);

			if (question != null) {
				System.out.println("\nQuestion " + (i + 1) + ": " + question.getQuestion());
				System.out.println("Your answer: " + (answer.userAnswer == ' ' ? "Skipped" : answer.userAnswer));
